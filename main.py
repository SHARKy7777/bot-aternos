...
ANNOUNCEMENT_CHANNEL_ID = 0  # ← Change ça avec l'ID du salon Discord
PREFIX = "!"
...
async def server_monitor():
    """Surveille le serveur toutes les 3 minutes"""
    global previous_status
    
    status = check_server_status()
    current_online = status["online"]
    
    # Change le statut du bot Discord
    if current_online:
    ...
    # Annonce les changements dans le salon
            ...
            if channel:
                if current_online:
                    embed = discord.Embed(
                        ...
                        description=f"Le serveur **{SERVER_ADDRESS}** est maintenant accessible.",
...
async def slash_status(interaction: discord.Interaction):
    ...
    if status["online"]:
        embed = discord.Embed(
            title=f"🎮 Serveur — {SERVER_ADDRESS}",
            color=discord.Color.green()
        )
...
async def slash_setchannel(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    embed = discord.Embed(
        ...
        description=f"Les annonces de statut seront envoyées dans {channel.mention}",
        color=discord.Color.green()
    )
    ...
    test_embed = discord.Embed(
        title="🔔 Annonces configurées",
        description="Ce salon recevra les notifications de statut du serveur Minecraft.",
...
async def slash_help(interaction: discord.Interaction):
    ...
    embed.add_field(
        name="/status",
        value="Affiche le statut actuel du serveur",
        inline=False
    )
    embed.add_field(
        name="/setchannel",
        value="Définir le salon pour les annonces (propriétaire)",
        inline=False
    )
    embed.add_field(
        name="/forcecheck",
        value="Forcer une vérification immédiate (propriétaire)",
        inline=False
    )
...
## Mets à jour aussi `requirements.txt` :
```
discord.py==2.3.2
python-dotenv==1.0.0
mcstatus==11.1.1
